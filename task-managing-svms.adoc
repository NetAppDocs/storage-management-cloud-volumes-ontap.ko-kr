...
[role="lead"]
스토리지 VM(SVM)은 클라이언트에게 스토리지 및 데이터 서비스를 제공하는 ONTAP 내에서 실행되는 가상 머신입니다. 이를 _SVM_ 또는 _vserver_라고도 합니다. Cloud Volumes ONTAP는 기본적으로 하나의 스토리지 VM으로 구성되지만, 일부 구성에서는 추가 스토리지 VM을 지원합니다.
