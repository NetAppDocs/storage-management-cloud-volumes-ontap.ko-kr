...
.이 작업에 관하여
전환 도구는 NetApp 지원 사이트에서 사용할 수 있으며 다음과 같은 산출물을 생성합니다.
...
+
.프라이빗 모드 배포의 경우, 다음 추가 필수 조건을 충족해야 합니다.
[%collapsible]
====
...
** 개인 모드 배포를 위해 네트워크 구성을 완료하고 작업자 풀을 생성했는지 확인하십시오. link:reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments["프라이빗 모드 배포를 위한 Infrastructure Manager 구성"]을 참조하십시오.
...
[%collapsible]
====
 `--worker-pool` 매개변수를 지정하여 프라이빗 모드 배포에서 툴을 실행합니다. 작업자 풀 구성에 대해서는 link:reference-networking-gcp.html#infrastructure-manager-configuration-for-private-mode-deployments["프라이빗 모드 배포를 위한 Infrastructure Manager 구성"]을 참조하십시오.
