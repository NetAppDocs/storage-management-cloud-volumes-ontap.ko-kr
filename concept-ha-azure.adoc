...
디스크:: 고객 데이터는 프리미엄 스토리지 페이지 블롭에 저장됩니다. 각 노드는 다른 노드의 스토리지에 접근할 수 있습니다. 추가 스토리지도 link:reference-configs-azure.html#azure-ha-pair["부트, 루트 및 코어 데이터"]에 필요합니다.
저장 계정::
+
--
...
NOTE: Cloud Volumes ONTAP 9.15.0P1 버전부터는 새로운 고가용성 쌍 배포에서 Azure 페이지 Blob이 더 이상 지원되지 않습니다. 기존 고가용성 쌍 배포에서 Azure 페이지 Blob을 사용하는 경우 Edsv4 시리즈 VM 및 Edsv5 시리즈 VM과 같은 최신 VM 인스턴스 유형으로 마이그레이션할 수 있습니다. link:reference-configs-azure.html#ha-pairs["Azure에서 지원되는 구성에 대해 자세히 알아보세요."].
...
NOTE: 위의 기준을 충족하는 환경에 대해 프리미엄 SSD 관리형 디스크를 선택하면 콘솔에서 프리미엄 SSD v2 관리형 디스크를 자동으로 배포합니다. Premium SSD v1 Managed Disks로 전환할 수 없습니다.

디스크:: 고객 데이터는 로컬 중복 스토리지(LRS) 관리 디스크에 저장됩니다. 각 노드는 다른 노드의 스토리지에 접근할 수 있습니다. 추가 스토리지도 link:reference-configs-azure.html#azure-ha-pair["부팅, 루트, 파트너 루트, 코어 및 NVRAM 데이터"]에 필요합니다.
...
디스크:: 고객 데이터는 영역 중복 스토리지(ZRS) 관리 디스크에 저장됩니다. 각 노드는 다른 노드의 스토리지에 접근할 수 있습니다. 추가 스토리지도 link:reference-configs-azure.html#azure-ha-pair["부트, 루트, 파트너 루트 및 코어 데이터"]에 필요합니다.
저장 계정:: 스토리지 계정은 진단 로그를 처리하고 Blob 스토리지로 계층화하기 위해 관리형 디스크 기반 배포에 사용됩니다.
...
NOTE: Cloud Volumes ONTAP는 프로브 처리를 위해 액티브-패시브 모드의 로드 밸런서를 사용합니다. Azure에서는 활성 노드만 프로브에 응답합니다. 패시브 노드는 X 표시와 함께 비활성 상태로 표시될 수 있습니다. 이는 정상적인 동작이며 우려할 필요가 없습니다.

.관련 링크
link:task-azure-high-availability-mode.html["Azure에서 Cloud Volumes ONTAP 에 대해 고가용성 모드 활성화"^]
