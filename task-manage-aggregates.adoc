...
. 필요한 애그리게이트에 대해 관리 작업을 위한 image:icon-action.png["추가 작업을 위한 가로 점 세 개가 표시된 아이콘입니다."] 아이콘을 클릭하십시오.
+
image:screenshot_aggr_menu_options.png["집계 메뉴 옵션의 스크린샷입니다."]

. image:icon-action.png["추가 작업을 위한 가로 점 세 개가 표시된 아이콘입니다."] 메뉴에서 제공되는 옵션을 사용하여 애그리게이트를 관리합니다.
...
. image:icon-action.png["추가 작업을 위한 가로 점 세 개가 표시된 아이콘입니다."] 메뉴에서 *용량 증가*를 클릭합니다.
