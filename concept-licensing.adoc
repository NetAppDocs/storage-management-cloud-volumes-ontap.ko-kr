...
[role="lead"]
Cloud Volumes ONTAP에 사용할 수 있는 라이센스 모델이 여러 가지 있습니다. 각 옵션을 통해 필요에 맞는 사용량 기반 모델을 선택할 수 있습니다.

클라우드 마켓플레이스 또는 NetApp(BYOL, Bring-Your-Own License)을 통해 라이선스 및 구독을 구매하거나 갱신할 수 있습니다. 마켓플레이스에서는 연간 구독 및 종량제(PAYGO) 옵션을 제공합니다. 자세한 내용은 link:concept-license-management.html["Cloud Volumes ONTAP 의 용량 기반 라이선스에 대해 자세히 알아보세요."]를 참조하십시오.

BYOL 라이선스는 NetApp에서 구매할 수 있으며 모든 클라우드 공급자에서 사용할 수 있습니다. BYOL 라이선스 사용 가능 여부에 대한 자세한 내용은 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP 에 대한 BYOL 라이선싱의 제한된 가용성"^]을 참조하십시오.
...
신규 고객은 용량 기반 연간 마켓플레이스 구독 또는 Keystone 구독을 통해 요금을 지불하는 방식을 선택할 수 있습니다. 프라이빗 모드의 구축 환경 등 일부 구축 환경에서는 이전 세대 노드 기반 BYOL 라이선스도 사용할 수 있습니다.

용량 기반 라이선싱:: NetApp 계정에서 여러 Cloud Volumes ONTAP 시스템에 대한 비용을 프로비저닝된 용량에 따라 지불합니다. 추가 클라우드 데이터 서비스를 구매할 수 있는 기능이 포함되어 있습니다. 용량 기반 라이선스의 소비 모델 또는 구매 옵션에 대한 자세한 내용은 다음을 참조하십시오.link:concept-licensing-charging.html["용량 기반 라이선스에 대해 자세히 알아보세요"] .
...
* link:reference-configs-azure.html["Azure에서 지원되는 구성"]
...
* link:reference-configs-gcp.html["Google Cloud에서 지원되는 구성"^]
...
NOTE: 백업 및 복구 서비스를 이용하려면 사용량 기반 요금제(PAYGO) 구독이 필요하지만, 이 서비스를 사용하는 데에는 요금이 부과되지 않습니다. 백업 및 복구 서비스 라이선스 설정에 대한 자세한 내용은 https://docs.netapp.com/us-en/data-services-backup-recovery/task-licensing-cloud-backup.html["백업 및 복구에 대한 라이선싱 설정"^]을 참조하십시오.
...
https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html#impact-on-cloud-volumes-ontap["콘솔 에이전트에 대해 알아보세요"^].


NOTE: NetApp은 BYOL 라이선스의 구매, 연장 및 갱신을 제한합니다. 자세한 내용은 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPP 에 대한 BYOL 라이선싱의 제한된 가용성"^]을 참조하십시오.
...
Keystone 구독은 HA 쌍에서 지원됩니다. 현재 이 라이센스 옵션은 단일 노드 시스템에서는 지원되지 않습니다.
...
NOTE: 프라이빗 클라우드에서 Cloud Volumes ONTAP의 프라이빗 모드 배포.
...
| 라이센스 유형 | EOA 이후의 영향 
 ...
 a| 
NetApp에서 구매하거나 BYOL한 유효한 노드 기반 라이선스입니다.
 a| 
...
이 라이선스를 사용하여 새로운 Cloud Volumes ONTAP 시스템을 배포할 수 없습니다.  기존 시스템은 계속 작동할 수 있지만 EOS 날짜 이후에는 시스템에 대한 지원이나 업데이트를 받을 수 없습니다.
 ...
 a| 
유효한 노드 기반 라이선스이며, 종량제 구독 방식입니다.
 a| 
EOS 날짜 이후에는 용량 기반 라이선스로 전환할 때까지 NetApp 지원을 받을 수 없습니다.

|===
.예외
NetApp은 AWS의 Cloud Volumes ONTAP 중국 지역 배포 등 특별한 고려가 필요한 특정 상황을 인지하고 있으며, 노드 기반 라이센스의 EOA 및 EOS는 다음과 같은 경우에 적용되지 않을 수 있습니다.
...
콘솔을 사용하면 라이선스 변환 툴을 통해 노드 기반 라이선스를 용량 기반 라이선스로 원활하게 변환할 수 있습니다. 노드 기반 라이선싱의 EOA에 대한 자세한 내용은 link:concept-licensing.html#end-of-availability-of-node-based-licenses["노드 기반 라이선스 제공 종료"]을 참조하십시오.
...
##마켓플레이스 구독 또는 노드 기반 라이선스가 있는 비공개 배포를 사용하는 경우 Console의 라이선스 변환 툴을 사용하여 link:task-convert-node-capacity.html["노드 기반 라이선스를 용량 기반 라이선스로 전환"] 할 수 있습니다.##
...
.관련 링크
* link:concept-licensing-charging.html["용량 기반 라이선스에 대해 자세히 알아보세요"]
* link:task-manage-capacity-licenses.html["Cloud Volumes ONTAP 에 대한 용량 기반 라이선싱 관리"]
* link:task-manage-node-licenses.html["노드 기반 라이선싱 관리"]
* link:task-convert-node-capacity.html["노드 기반 라이선스를 용량 기반 라이선스로 변환"]
* link:task-manage-keystone.html["Keystone 구독 관리"]
