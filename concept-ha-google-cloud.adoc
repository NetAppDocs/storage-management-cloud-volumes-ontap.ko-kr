...
* 단일 가용성 영역(AZ) 또는 다중 가용성/3개 영역(권장).
+
...
기존에는 Google Cloud의 Cloud Volumes ONTAP HA 구성 시 미디에이터 인스턴스를 사용하여 장애 조치 작업을 관리하거나 노드 상태를 모니터링하는 비공유 모델을 사용했습니다. 이를 위해서는 Google Cloud에 추가 VM을 생성하고 각 노드에 대해 두 세트의 디스크를 구성해야 합니다.
...
=== 단일 AZ HA 구성을 위한 공유 디스크

Cloud Volumes ONTAP 9.19.1 이상은 단일 AZ에 대한 공유 HA 구성을 지원합니다. 공유 HA 아키텍처에서는 두 노드가 디스크를 공유할 수 있으며, 이 경우 애그리게이트 생성 방식이 변경됩니다. 두 개의 VM에 대해 각각 디스크를 생성하는 대신, 하나의 디스크를 생성하여 VM에 연결합니다. 공유 HA는 디스크 미러링이 필요하지 않습니다.
...
여러 가용성 영역(권장):: 3개 영역에 걸쳐 HA 구성을 배포하면 영역 내에서 장애가 발생하더라도 지속적인 데이터 가용성이 보장됩니다.  단일 영역을 사용하는 것에 비해 쓰기 성능은 약간 낮지만 최소한입니다.
단일 가용성 영역:: 단일 가용성 영역(AZ)에 배포된 경우, Cloud Volumes ONTAP HA 구성은 분산 배치 정책을 사용합니다. 이 정책은 별도의 영역을 사용하지 않고도 장애 격리를 달성하면서 HA 구성이 영역 내 단일 장애 지점으로부터 보호되도록 합니다.
+
--
