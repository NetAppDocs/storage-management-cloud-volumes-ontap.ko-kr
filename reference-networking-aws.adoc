...
Cloud Volumes ONTAP 배포 중에 AWS 중재자 프록시 필드는 서버 주소를 허용합니다. IP 주소 대신 정규화된 도메인 이름(FQDN)을 입력하면 AWS VPC가 사용자 지정 DNS 서버로 구성되지 않는 한 FQDN이 확인되지 않습니다. 이는 AWS에서 기본적으로 제공하는 DNS가 공용 DNS 레코드와 내부 AWS 호스트만 확인할 수 있고 고객 프록시 서버와 같은 외부 FQDN은 확인할 수 없기 때문입니다. 이러한 상황이 발생하는 경우 다음 두 가지 옵션이 있습니다.

* FQDN 대신 프록시 서버의 IP 주소를 미디에이터 프록시 필드에 직접 추가하세요.
* 사용자 지정 DNS 서버 주소를 포함하는 DHCP 옵션 세트를 VPC에서 구성하십시오. 이렇게 하면 배포 중에 프록시 서버 필드에 FQDN을 입력할 때 AWS가 FQDN을 확인할 수 있습니다. 자세한 내용은 https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["AWS 문서"^]를 참조하십시오.
...
HA 쌍은 단일 노드 시스템보다 더 많은 IP 주소가 필요합니다. 이러한 IP 주소는 다음 이미지에 표시된 것처럼 여러 이더넷 인터페이스에 분산됩니다.
...
TIP: 콘솔 에이전트에 대한 정보를 찾고 계십니까? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["콘솔 에이전트에 대한 보안 그룹 규칙 보기"^]
...
EBS를 성능 계층으로, Amazon S3를 용량 계층으로 사용하려면 Cloud Volumes ONTAP이 S3에 연결되어 있어야 합니다. 이 연결을 제공하는 가장 좋은 방법은 S3 서비스에 대한 VPC 엔드포인트를 생성하는 것입니다. 지침은 https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["AWS 설명서: 게이트웨이 엔드포인트 생성"^]을 참조하십시오.
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["콘솔 에이전트에 대한 네트워킹 요구 사항 보기"^]
