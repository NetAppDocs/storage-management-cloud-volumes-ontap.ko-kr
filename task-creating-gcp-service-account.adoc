...
[role="lead"]
Cloud Volumes ONTAP에는 두 가지 이유로 Google Cloud 서비스 계정이 필요합니다. 첫 번째는 link:concept-data-tiering.html["데이터 계층화"]을 활성화하여 콜드 데이터를 Google Cloud의 저렴한 오브젝트 스토리지로 계층화하는 경우입니다. 두 번째는  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^]을 활성화하여 볼륨을 저렴한 오브젝트 스토리지에 백업하는 경우입니다.
