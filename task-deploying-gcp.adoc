...
+
 https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-gcp.html#flash-cache-support["Flash Cache"^]와 link:concept-write-speed.html["쓰기 속도"]에 대해 자세히 알아보세요.
...
이 시스템에 사용할 Google 프로젝트와 구독을 선택하세요. 이 서비스 계정은 Console에 설정한 계정입니다. 자세한 내용은 link:https://docs.netapp.com/us-en/bluexp-setup-admin/task-quick-start-connector-google.html["콘솔 설정 문서"^]을 참조하세요. Console은 프로젝트를 사용하여 Cloud Volumes ONTAP 시스템을 생성하고, 구독을 사용하여 요금 및 사용량을 계산합니다. 드롭다운 목록에 다른 프로젝트가 표시되지 않으면 아직 서비스 계정을 다른 프로젝트에 연결하지 않은 것입니다. Google Cloud Console로 이동하여 IAM 서비스를 열고 해당 프로젝트를 선택하세요. Console에 사용하는 역할을 가진 서비스 계정을 해당 프로젝트에 추가하세요. 각 프로젝트에 대해 이 단계를 반복해야 합니다.

*구독 추가*를 클릭하여 선택한 자격 증명을 구독과 연결합니다.

종량제 방식의 Cloud Volumes ONTAP 시스템을 생성하려면 Google Cloud Marketplace에서 Cloud Volumes ONTAP 구독과 연결된 Google Cloud 프로젝트를 선택해야 합니다. 콘솔 설정 문서를 참조하세요:  https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-gcp-accounts.html["Google Cloud 자격 증명과 마켓플레이스 구독 연결"^].
