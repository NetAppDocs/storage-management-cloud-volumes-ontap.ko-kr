...
+
** 다음이 있어야 합니다 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["시스템과 연결된 콘솔 에이전트"^].
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["항상 콘솔 에이전트를 실행 상태로 두어야 합니다."^].
** 콘솔 에이전트와 연결된 서비스 계정입니다 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["필요한 권한이 있어야 합니다"^].
...
. 메시지가 표시되면, https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["콘솔 에이전트 생성"^].
...
| 서비스 계정 이름 | Cloud Volumes ONTAP과 함께 link:concept-data-tiering.html["데이터 계층화"] 또는  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^]을(를) 사용할 계획이라면 *서비스 계정*을 활성화하고 미리 정의된 스토리지 관리자 역할을 가진 서비스 계정을 선택해야 합니다. link:task-creating-gcp-service-account.html["서비스 계정을 만드는 방법을 알아보세요"^]. 
...
NOTE: 이것은 콘솔용으로 설정한 서비스 계정입니다. link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["이 페이지에 설명된 대로"^].

*구독 추가*를 클릭하여 선택한 자격 증명을 구독과 연결합니다.

종량제 방식의 Cloud Volumes ONTAP 시스템을 생성하려면 Google Cloud 마켓플레이스에서 Cloud Volumes ONTAP 구독과 연결된 Google Cloud 프로젝트를 선택해야 합니다. 을 참조하십시오. https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Google Cloud 자격 증명과 마켓플레이스 구독 연결"^]
...
. *사전 구성된 패키지*: 패키지 중 하나를 선택하여 Cloud Volumes ONTAP 시스템을 빠르게 배포하거나 *나만의 구성 생성*을 클릭하십시오. 사전 구성된 패키지는 선택한 Cloud Volumes ONTAP 버전에 따라 다릅니다. 예를 들어 Cloud Volumes ONTAP 9.18.1 이상 버전의 경우 NetApp Console에 Hyperdisk Balanced 디스크를 포함한 C3 VM이 포함된 패키지가 표시됩니다. 워크로드 요구 사항에 따라 IOPS 및 처리량 매개변수와 같은 구성을 수정할 수 있습니다.
+
...
+
디스크 유형 및 크기 선택에 대한 도움말은 link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["Google Cloud에서 시스템 크기 조정"^]을 참조하십시오.

. *플래시 캐시, 쓰기 속도 및 WORM*:
+
.. 필요한 경우 *Flash Cache*를 활성화하거나 *Normal* 또는 *High* 쓰기 속도를 선택하십시오.
+
link:reference-configs-gcp.html#flash-cache-support["Flash Cache"] 및 link:concept-write-speed.html["쓰기 속도"]에 대해 자세히 알아보십시오.

+

NOTE: *고속* 쓰기 속도 옵션을 선택하면 높은 쓰기 속도와 최대 전송 단위(MTU) 8,896바이트를 사용할 수 있습니다. 또한, 8,896바이트의 높은 MTU를 사용하려면 배포 시 VPC-1, VPC-2 및 VPC-3을 선택해야 합니다. VPC-1, VPC-2 및 VPC-3에 대한 자세한 내용은  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["VPC-1, VPC-2 및 VPC-3에 대한 규칙"^]를 참조하십시오.
...
** Google Cloud 계정 추가에 대한 도움말은  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["9.6을 사용하여 데이터 계층화를 위한 Google Cloud 계정 설정 및 추가"^]을 참조하십시오.
...
| NTP 서버 | Active Directory DNS를 사용하여 NTP 서버를 구성하려면 *Active Directory 도메인 사용*을 선택하십시오. 다른 주소를 사용하여 NTP 서버를 구성해야 하는 경우 API를 사용해야 합니다. 자세한 내용은  https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console 자동화 문서"^]을 참조하십시오. NTP 서버는 CIFS 서버를 생성할 때만 구성할 수 있습니다. CIFS 서버를 생성한 후에는 구성할 수 없습니다. 
|===
...
. *시스템 세부 정보*: 프로젝트를 선택하고 클러스터 이름을 지정하고, 선택적으로 Service Account를 선택하고, 선택적으로 레이블을 추가한 다음 자격 증명을 지정합니다.
+
다음 표에서는 지침이 필요할 수 있는 필드를 설명합니다.
...
| Google Cloud 프로젝트 및 마켓플레이스 구독  a| 
이 시스템에 사용할 Google 프로젝트와 구독을 선택하세요. 이 서비스 계정은 Console에 설정한 계정입니다. 자세한 내용은 link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["콘솔 설정 문서"^]을 참조하세요. Console은 프로젝트를 사용하여 Cloud Volumes ONTAP 시스템을 생성하고, 구독을 사용하여 요금 및 사용량을 계산합니다. 드롭다운 목록에 다른 프로젝트가 표시되지 않으면 아직 서비스 계정을 다른 프로젝트에 연결하지 않은 것입니다. Google Cloud Console로 이동하여 IAM 서비스를 열고 해당 프로젝트를 선택하세요. Console에 사용하는 역할을 가진 서비스 계정을 해당 프로젝트에 추가하세요. 각 프로젝트마다 이 단계를 반복해야 합니다.

*구독 추가*를 클릭하여 선택한 자격 증명을 구독과 연결합니다.

종량제 방식의 Cloud Volumes ONTAP 시스템을 생성하려면 Google Cloud Marketplace에서 Cloud Volumes ONTAP 구독과 연결된 Google Cloud 프로젝트를 선택해야 합니다. 콘솔 설정 문서를 참조하세요:  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["Google Cloud 자격 증명과 마켓플레이스 구독 연결"^].
...
| 서비스 계정 이름 | link:concept-data-tiering.html["NetApp Cloud Tiering"] 또는  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["백업 및 복구"^] 서비스를 사용하려면 *서비스 계정* 스위치를 활성화한 다음 미리 정의된 스토리지 관리자 역할을 가진 서비스 계정을 선택해야 합니다. 


| 라벨 | 라벨은 Google Cloud 리소스에 대한 메타데이터입니다.  콘솔은 Cloud Volumes ONTAP 시스템 및 해당 시스템과 연결된 Google Cloud 리소스에 레이블을 추가합니다.  시스템을 생성할 때 사용자 인터페이스에서 최대 4개의 라벨을 추가할 수 있으며, 시스템을 생성한 후에 라벨을 더 추가할 수 있습니다.  API는 시스템을 생성할 때 레이블을 4개로 제한하지 않습니다.  라벨에 대한 정보는 다음을 참조하세요. https://cloud.google.com/compute/docs/labeling-resources["Google Cloud 문서: 리소스 레이블 지정"^] . 


| ONTAP 사용자 이름 및 비밀번호 | 이는 Cloud Volumes ONTAP 클러스터 관리자 계정의 자격 증명입니다.  이러한 자격 증명을 사용하면 ONTAP System Manager나 ONTAP CLI를 통해 Cloud Volumes ONTAP 에 연결할 수 있습니다.  기본 _admin_ 사용자 이름을 유지하거나 사용자 지정 사용자 이름으로 변경하세요. 
|===
. *서비스*: 이 시스템에서 사용할 서비스를 선택하세요.  백업 및 복구를 선택하거나 NetApp Cloud Tiering 사용하려면 3단계에서 서비스 계정을 지정해야 합니다.
+
...
. *배포 토폴로지*: HA 구성을 위해 여러 가용 영역(AZ)(권장) 또는 단일 AZ를 선택합니다.
+
link:concept-ha-google-cloud.html["HA 배포 모델에 대해 자세히 알아보세요"^] .

. *네트워크 구성 및 연결*: 관련 지역과 영역을 선택합니다. HA 구성을 위해 서로 다른 VPC 4개를 선택하고, 각 VPC에서 서브넷을 선택한 다음 방화벽 정책을 선택합니다.
+

NOTE: Cloud Volumes ONTAP 9.19.1 이상은 link:concept-ha-google-cloud.html["단일 AZ HA 구성을 위한 공유 디스크"]을 지원하므로 미디에이터 인스턴스가 선택 사항입니다. 이 구성에서는 데이터 복제를 위한 미디에이터 영역, 해당 VPC 및 서브넷을 입력할 필요가 없습니다.
...
+
패키지 중 하나를 선택하면 볼륨만 지정한 다음 구성을 검토하고 승인하면 됩니다. 사전 구성된 패키지는 선택한 Cloud Volumes ONTAP 버전 및 구성 유형에 따라 다릅니다. 예를 들어 Cloud Volumes ONTAP 9.19.1 이상 버전에서 단일 AZ 배포를 선택하면 C3 시리즈 머신과 Hyperdisk Balanced 디스크가 포함된 사전 구성된 패키지를 볼 수 있습니다. 이전 버전 및 여러 AZ의 경우 n2-standard 머신과 Persistent Disks를 선택할 수 있습니다. 워크로드 요구 사항에 따라 IOPS 및 처리량 매개변수와 같은 구성을 수정할 수 있습니다.
...
NOTE: n2-standard-16, n2-standard-32, n2-standard-48 및 n2-standard-64 인스턴스 유형에서는 *고속* 쓰기 속도 옵션을 통해 높은 쓰기 속도와 최대 전송 단위(MTU) 8,896바이트를 사용할 수 있습니다. 또한, 8,896바이트의 높은 MTU를 사용하려면 배포 시 VPC-1, VPC-2 및 VPC-3을 선택해야 합니다. 높은 쓰기 속도와 8,896바이트의 MTU는 특정 기능에 종속적이며, 구성된 인스턴스 내에서 개별적으로 비활성화할 수 없습니다. VPC-1, VPC-2 및 VPC-3에 대한 자세한 내용은  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["VPC-1, VPC-2 및 VPC-3에 대한 규칙"^]을 참조하십시오.
