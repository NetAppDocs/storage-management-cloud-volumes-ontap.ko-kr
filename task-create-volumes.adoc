...
* iSCSI 볼륨을 생성하면 콘솔에서 자동으로 LUN을 생성합니다.  볼륨당 하나의 LUN만 생성하도록 간소화하여 관리가 필요 없습니다.  볼륨 생성 후 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-connect-lun.html["IQN을 사용하여 호스트에서 LUN에 연결합니다."^].
* ONTAP 시스템 관리자나 ONTAP CLI에서 추가 LUN을 생성할 수 있습니다.


ifdef::aws[]

* AWS에서 CIFS를 사용하려면 DNS와 Active Directory를 설정해야 합니다. 자세한 내용은 link:reference-networking-aws.html["AWS의 Cloud Volumes ONTAP 에 대한 네트워킹 요구 사항"]을 참조하십시오.
...
.특정 집계
--
.. *Aggregates* 탭에서 필요한 애그리게이트로 이동하여 image:icon-action.png["추가 작업을 위한 가로 점 세 개가 표시된 아이콘입니다."] 아이콘을 클릭합니다.
...
NTP 서버:: Active Directory DNS를 사용하여 NTP 서버를 구성하려면 *Active Directory 도메인 사용*을 선택하십시오. 다른 주소를 사용하여 NTP 서버를 구성해야 하는 경우 API를 사용해야 합니다. 자세한 내용은 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console 자동화 문서"^]을 참조하십시오.
+
--
...
+
*** link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#size-your-system-in-aws["AWS에서 시스템 크기 조정"^]
...
* link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Azure에서 시스템 크기 조정"^]
...
* link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-gcp.html#size-your-system-in-gcp["Google Cloud에서 시스템 크기 조정"^]
