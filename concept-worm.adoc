...
WORM 기능은 마켓플레이스 구독 및 BYOL(Bring-Your-Own License) 라이선스에서 추가 비용 없이 사용할 수 있습니다. 현재 라이선스에 WORM을 추가하려면 NetApp 영업 담당자에게 문의하십시오.
...
애플리케이션을 사용하여 NFS 또는 CIFS를 통해 파일을 WORM에 커밋하거나 ONTAP CLI를 사용하여 파일을 자동으로 WORM에 커밋할 수 있습니다. 또한 로그 정보와 같이 점진적으로 기록되는 데이터를 보존하기 위해 WORM 추가 가능 파일을 사용할 수 있습니다. 자세한 내용은 https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["ONTAP SnapLock을 사용하여 WORM에 파일 커밋"^]을 참조하십시오.

Cloud Volumes ONTAP 시스템에서 WORM 스토리지를 활성화한 후에는 WORM 스토리지의 모든 관리를 위해 ONTAP CLI를 사용해야 합니다. 자세한 내용은 link:https://docs.netapp.com/us-en/ontap/snaplock/index.html["SnapLock 에 대한 ONTAP 문서"^]을 참조하십시오.
...
* link:concept-licensing-charging.html["Cloud Volumes ONTAP 의 라이선싱 및 요금 청구"]
