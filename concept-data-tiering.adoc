...
Google Cloud에서 데이터 계층화를 사용하도록 설정하면 Cloud Volumes ONTAP은 자주 사용되는 데이터의 성능 계층으로 영구 디스크를 사용하고, 사용되지 않는 데이터의 용량 계층으로 Google Cloud Storage 버킷을 사용합니다. 시스템을 생성할 때 스토리지 계층을 선택할 수 있으며, 이후 언제든지 변경할 수 있습니다. 스토리지 클래스 변경에 대한 자세한 내용은 link:task-tiering.html["비활성 데이터를 저비용 객체 스토리지로 계층화"]을 참조하십시오.
...
--
스토리지 클래스:: Google Cloud는  https://cloud.google.com/storage/docs/storage-classes["스토리지 클래스"^]비용 및 데이터 가용성을 관리하는 데 사용됩니다. 데이터 액세스 시 비용이 증가하므로 예상 데이터 액세스 빈도에 따라 스토리지 클래스를 선택하는 것이 좋습니다. 계층형 데이터의 기본 스토리지 클래스는 _Standard Storage_입니다. 수동으로 변경하지 않는 한 모든 비활성 데이터는 기본 스토리지 클래스를 유지합니다. 비활성 데이터의 스토리지 클래스를 변경하려면  https://docs.cloud.google.com/storage/docs/managing-lifecycles["Google Cloud 문서: 객체 수명 주기 관리"^]을 참조하십시오.
+
--
데이터 계층화를 위한 스토리지 클래스는 볼륨별이 아닌 시스템 전체에 적용됩니다.
