...
* Cloud Volumes ONTAP의 무료 라이선스는 최대 500GiB의 프로비저닝 용량까지 제공되며, NetApp 지원은 제공되지 않고 만료일도 없습니다. 500GiB를 초과하는 용량을 사용하려면 유효한 라이선스 권한이 필요하며 추가 요금이 발생할 수 있습니다. Cloud Volumes ONTAP의 전체 기능과 고급 관리 기능을 활용하려면 Cloud Volumes ONTAP 시스템을 NetApp Console에 연결하십시오. NetApp Console을 통한 검색 및 관리에는 NetApp Console 계정과 연결된 유효한 Azure Marketplace 구독이 필요합니다.
...
.시작하기 전에
* NetApp Console에서 배포된 시스템을 검색하고 관리하려면 NetApp Console 계정이 유효한 Azure Marketplace 구독과 연결되어 있는지 확인하십시오. NetApp Console에서 시스템을 검색하려면 유효한 구독이 필요합니다.
