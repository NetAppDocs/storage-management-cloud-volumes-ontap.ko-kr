...
[role="lead"]
Azure Marketplace 직접 배포를 사용하면 Cloud Volumes ONTAP을 빠르고 쉽게 배포할 수 있습니다. Azure Marketplace에서 몇 번의 클릭만으로 Cloud Volumes ONTAP을 신속하게 배포하고 환경에서 핵심 기능과 역량을 살펴볼 수 있습니다.
...
.이 작업에 관하여
Azure Marketplace 직접 배포를 사용하여 배포된 Cloud Volumes ONTAP 시스템은 다음과 같은 속성을 갖습니다. Azure Marketplace를 통해 배포된 독립 실행형 인스턴스의 기능은 NetApp Console에서 검색될 때 변경됩니다.

* 최신 Cloud Volumes ONTAP 버전(9.16.1 이상).
* Cloud Volumes ONTAP의 무료 라이선스는 최대 500GiB의 프로비저닝 용량까지 제공되며, NetApp 지원은 제공되지 않고 만료일도 없습니다. 500GiB를 초과하는 용량을 사용하려면 유효한 라이선스 권한이 필요하며 요금이 부과될 수 있습니다. Cloud Volumes ONTAP의 전체 기능과 고급 관리 기능을 활용하려면 Cloud Volumes ONTAP 시스템을 콘솔에 연결하십시오. 콘솔을 통한 검색 및 관리에는 콘솔 계정과 연결된 유효한 Azure Marketplace 구독이 필요합니다.
...
.시작하기 전에
* 콘솔에서 배포된 시스템을 검색하고 관리하려면 콘솔 계정이 유효한 Azure Marketplace 구독과 연결되어 있는지 확인하십시오. 콘솔에서 시스템을 검색하려면 유효한 구독이 필요합니다.
...
Azure Marketplace를 통해 직접 배포된 Cloud Volumes ONTAP 시스템에는 NetApp의 지원이 포함되지 않습니다. 배포 중에 문제가 발생하는 경우 직접 문제를 해결할 수 있습니다.

.단계
. Azure Marketplace 사이트에서 *부팅 진단 > 시리얼 로그*로 이동합니다.
. 직렬 로그를 다운로드하고 조사하세요.
. 문제 해결을 위해서는 제품 설명서와 지식 기반(KB) 문서를 참조하세요.
+
** https://learn.microsoft.com/en-us/partner-center/["Azure Marketplace 설명서"]
...
Azure Marketplace 직접 배포를 사용하여 배포한 Cloud Volumes ONTAP 시스템을 콘솔의 *시스템* 페이지에서 검색하고 관리할 수 있습니다. 콘솔 에이전트는 시스템을 검색하고 추가하며 필요한 라이선스를 적용하여 해당 시스템에 대한 콘솔의 모든 기능을 활성화합니다. PSSD v2 관리 디스크를 사용하는 단일 AZ의 원래 HA 구성은 유지되며, 시스템은 원래 배포와 동일한 Azure 구독 및 리소스 그룹에 등록됩니다.

.이 작업에 관하여
Azure Marketplace 직접 배포를 사용하여 배포된 Cloud Volumes ONTAP 시스템을 검색하면 Console 에이전트는 다음 작업을 수행합니다.
...
.시작하기 전에
Azure Marketplace에서 배포가 완료되었는지 확인하십시오. 콘솔 에이전트는 배포가 완료되어 시스템이 검색 가능한 상태일 때만 시스템을 검색할 수 있습니다.

.단계
콘솔에서는 기존 시스템을 검색하기 위한 표준 절차를 따릅니다. link:task-adding-systems.html["콘솔에 기존 Cloud Volumes ONTAP 시스템 추가"] .


CAUTION: 검색 중에 오류 메시지가 표시될 수 있지만 검색 프로세스가 완료될 때까지 무시해도 됩니다. 검색 중에 Azure Marketplace 포털에서 시스템에서 생성된 Cloud Volumes ONTAP 구성, 특히 시스템 태그를 수정하지 마십시오. 이러한 구성을 변경하면 예기치 않은 시스템 동작이 발생할 수 있습니다.
...
* https://docs.netapp.com/us-en/console-licenses-subscriptions/task-manage-subscriptions.html["새 마켓플레이스 구독 추가"^]
