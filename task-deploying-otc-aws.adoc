...
| 자격 증명 편집 | 이 Cloud Volumes ONTAP 시스템에 사용할 AWS 자격 증명 및 마켓플레이스 구독을 선택합니다. 선택한 자격 증명을 새 AWS 마켓플레이스 구독과 연결하려면 *구독 추가*를 클릭합니다. 구독은 연간 계약 또는 시간당 요금으로 Cloud Volumes ONTAP 비용을 지불하는 방식일 수 있습니다. NetApp에서 직접 라이선스를 구매한 경우(BYOL(사용자 라이선스 사용)) AWS 구독이 필요하지 않습니다. NetApp은 BYOL 라이선스의 구매, 연장 및 갱신을 제한했습니다. 자세한 내용은  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP 에 대한 BYOL 라이선싱의 제한된 가용성"^]을(를) 참조하십시오. https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["콘솔에 추가 AWS 자격 증명을 추가하는 방법을 알아보세요."^]. 
|===
+
