...
NOTE: Cloud Volumes ONTAP 시스템이 AWS Local Zone에 있는 경우 Amazon Simple Storage Service(Amazon S3)로의 데이터 계층화는 지원되지 않습니다. Local Zone 외부의 Amazon S3 버킷에 액세스하면 지연 시간이 길어지고 Cloud Volumes ONTAP 활동에 영향을 미치기 때문입니다.
...
link:reference-configs-aws.html#aws["AWS에서 Cloud Volumes ONTAP 시스템 데이터의 기본 디스크 보기"].
