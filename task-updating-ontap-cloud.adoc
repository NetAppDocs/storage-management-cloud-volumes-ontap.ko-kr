...
업그레이드할 수 있는 Cloud Volumes ONTAP 버전은 현재 실행 중인 버전에 따라 다릅니다. 다음 표의 각 릴리스에 있는 일반 버전 또는 패치 버전은 업그레이드 가능한 기본 버전을 나타냅니다. 사용 가능한 패치에 대한 자세한 내용은 각 릴리스의 https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-new.html["버전별 릴리스 노트"^]를 참조하십시오.
...
이 페이지에 설명된 방법 중 하나를 사용하여 소프트웨어를 업그레이드하려면 Cloud Volumes ONTAP를 NetApp 지원팀에 등록해야 합니다. 이는 종량제(PAYGO) 및 BYOL(Bring-Your-Own License) 모두에 적용됩니다. BYOL 시스템은 기본적으로 등록되어 있지만 link:task-registering.html["PAYGO 시스템 수동 등록"]해야 합니다.
...
link:https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-aws.html#disk-and-tiering-limits-by-ec2-instance["EC2 인스턴스별 디스크 및 계층화 제한 확인"^] link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-change-ec2-instance.html["Cloud Volumes ONTAP 의 EC2 인스턴스 유형 변경"^]
...
. 필요한 애그리게이트 타일에서 image:icon-action.png["추가 작업을 위한 가로 점 세 개가 표시된 아이콘입니다."] 아이콘을 클릭한 다음 *애그리게이트 세부 정보 보기*를 선택합니다.
+
...
. 시스템에서 image:icon-action.png["추가 작업을 위한 가로 점 세 개가 표시된 아이콘입니다."] 아이콘을 클릭한 다음 *Cloud Volumes ONTAP 업데이트*를 클릭합니다.
