...
summary: Cloud Volumes ONTAP 시스템이 제대로 작동하도록 Google Cloud 네트워킹을 설정합니다. 이 문서에서는 네트워킹 요구 사항, 프라이빗 IP 주소, VPC, 서브넷, 패킷 미러링, 방화벽 규칙, CVO에 대한 HA 구성 및 콘솔 에이전트 요구 사항을 다룹니다. 
---
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["콘솔 에이전트 배포에서 다루는 필수 공유 VPC 권한을 검토하세요."^]
...
콘솔 에이전트는 일상적인 작업을 위해 여러 엔드포인트와도 통신합니다. 엔드포인트에 대한 자세한 내용은  https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["콘솔 에이전트에서 연결된 엔드포인트 보기"^] 및  https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["콘솔 사용을 위한 네트워킹 준비"^]을 참조하십시오.
...
| \https://cloudbuild.googleapis.com/v1 (개인 모드 배포에만 해당) \https://cloudkms.googleapis.com/v1 \https://cloudresourcemanager.googleapis.com/v1/projects \https://compute.googleapis.com/compute/v1 \https://www.googleapis.com/compute/beta \https://www.googleapis.com/compute/v1/projects/ \https://www.googleapis.com/deploymentmanager/v2/projects \https://www.googleapis.com/storage/v1 \https://www.googleapis.com/upload/storage/v1 \https://config.googleapis.com/v1 \https://iam.googleapis.com/v1 \https://storage.googleapis.com/storage/v1 | Google Cloud(상업적 사용). | Google Cloud 서비스와의 통신. | 표준, 제한, 비공개 모드. | Cloud Volumes ONTAP Google Cloud 서비스와 통신하여 Google Cloud의 콘솔에 대한 특정 작업을 수행할 수 없습니다. 
|===
...
TIP: 콘솔 에이전트에 대한 정보를 찾고 계십니까? https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html["콘솔 에이전트에 대한 방화벽 규칙 보기"^]
...
=== 프라이빗 모드 또는 보호 환경에서의 Infrastructure Manager 구성

비공개 모드 또는 VPC 서비스 제어로 보호되는 프로젝트에 Cloud Volumes ONTAP 9.16.1 이상을 배포하려면 Cloud Volumes ONTAP이 Google에서 향후 지원을 중단할 예정인 Deployment Manager 대신 Google Cloud Infrastructure Manager를 배포 서비스로 사용할 수 있도록 몇 가지 구성을 변경해야 합니다.

.시작하기 전에
* Cloud Volumes ONTAP 시스템이 9.16.1 이상인지 확인하십시오. 그렇지 않은 경우 시스템을 업그레이드하십시오. 지침은 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html["Cloud Volumes ONTAP 업그레이드"]을 참조하십시오.
* Google Cloud API가 사용 설정되어 있는지 확인하세요.  https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["Google Cloud API 활성화"^]를 참조하십시오.
* Cloud Build API가 활성화되어 있는지 확인하십시오.  https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment["여기에서 Cloud Build API를 활성화하세요"^]을(를) 참조하십시오.
* Console 에이전트의 서비스 계정에 모든 표준 권한이 있는지 확인하십시오. 또한 서비스 계정에 `cloudbuild.workerpools.get` 및 `cloudbuild.workerpools.list` 권한이 있는지 확인하십시오.  https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions["콘솔 에이전트에 대한 Google Cloud 권한"^]을 참조하십시오.


.단계
. Cloud Volumes ONTAP 배포와 동일한 리전에 이 구성을 사용하여 프라이빗 워커 풀을 생성합니다. 프라이빗 워커 풀 생성에 대한 자세한 내용은  https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool["Google Cloud 문서: 프라이빗 풀 생성 및 관리"^] 및  https://cloud.google.com/build/pricing["Google Cloud Build 가격 책정"^]을 참조하십시오.
+
VPC 서비스 제어로 보호되는 프로젝트에 Cloud Volumes ONTAP을 배포하는 경우 콘솔 에이전트와 동일한 프로젝트에 프라이빗 워커 풀을 생성해야 합니다. 워커 풀은 실행 중 상태여야 합니다. Google Cloud의 기본 공용 Cloud Build 워커 풀은 서비스 경계 내에서 실행할 수 없습니다.
...
** 외부 IP 할당: False
** 네트워크: 기본 또는 프라이빗.
**  https://cloud.google.com/apis["Google Cloud API"^]에 액세스하도록 구성된 서브넷. 서브넷이 Google API에 액세스할 수 있도록 하려면 다음 단계를 수행하십시오.
+
... 서브넷에 대해 "Private Google Access"가 켜져 있는지 확인하십시오.
... *VPC Network 레벨 > Private Service Access 탭 > 서비스에 할당된 IP 범위*로 이동하십시오.
... *IP 범위 할당*을 선택하고 Google Compute Service에 대한 프라이빗 연결의 내부 IP 범위를 할당합니다.
... *서비스에 대한 프라이빗 연결*에서 *연결 생성*을 선택합니다.
... *연결된 서비스 공급자 = Google Cloud Platform*을 선택합니다.
... 이전 단계에서 생성한 프라이빗 연결 IP 범위에 대한 할당을 지정합니다.




. 이 워커 풀을 배포하고 Cloud Volumes ONTAP 관리를 위해 계속 실행 상태로 유지하세요. Google Cloud는 이 워커 풀을 사용하여 모든 Terraform 작업을 격리된 환경에서 실행합니다.
. 프라이빗 모드에서 Cloud Volumes ONTAP을 배포할 때 *GCP Worker Pool* 필드에서 이 워커 풀의 이름을 선택하십시오. 지침은 link:task-deploying-gcp.html["Google Cloud에서 Cloud Volumes ONTAP 실행"]을 참조하십시오.
+
VPC Service Controls로 보호되는 프로젝트에 Cloud Volumes ONTAP을 배포하는 경우 API를 사용하여 Cloud Volumes ONTAP 시스템을 생성하고 생성 요청 본문에  `workerPool` 매개변수를 포함해야 합니다.

+

NOTE: Cloud Volumes ONTAP 콘솔 UI를 사용하여 Cloud Volumes ONTAP을 배포할 때 워커 풀을 포함하는 것은 현재 지원되지 않습니다. VPC 서비스 제어로 보호되는 프로젝트의 경우 API가 Cloud Volumes ONTAP을 배포하는 유일한 지원 방법입니다.
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["콘솔 에이전트에 대한 네트워킹 요구 사항 보기"^]
...
Console 에이전트의 프록시 서버 구성에 대한 자세한 내용은 https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["프록시 서버를 사용하도록 콘솔 에이전트 구성"^]을 참조하십시오.
