...
이미지를 Cloud Storage로 내보내는 가장 좋은 방법은 다음을 사용하는 것입니다. https://cloud.google.com/sdk/gcloud/reference/compute/images/export["gcloud compute 이미지 내보내기 명령"^] .  이 명령은 제공된 이미지를 가져와서 tar와 gzip으로 압축된 disk.raw 파일로 변환합니다.  생성된 파일은 대상 URL에 저장되며, 확인을 위해 다운로드할 수 있습니다.
