...
용량 기반 라이선싱을 사용하는 경우, Cloud Volumes ONTAP 시스템의 최대 수는 NetApp 콘솔 계정당 24개로 제한됩니다.  _시스템_은 Cloud Volumes ONTAP HA 쌍, Cloud Volumes ONTAP 단일 노드 시스템 또는 사용자가 생성하는 추가 스토리지 VM입니다.  기본 스토리지 VM은 제한에 포함되지 않습니다.  이 제한은 모든 라이선스 모델에 적용됩니다.
...
*관리 > 라이선스 및 구독*에서 _Essentials Single Node_ 라이선스에 대해 50TiB가 청구된 것을 확인할 수 있습니다.
