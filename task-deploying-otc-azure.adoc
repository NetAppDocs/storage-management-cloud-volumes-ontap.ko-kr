...
---
= Azure의 고급 옵션을 사용하여 Cloud Volumes ONTAP 배포
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 이미 실행 중인 콘솔 에이전트입니다.
+
** 다음이 있어야 합니다. https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["시스템과 연결된 콘솔 에이전트"^]
...
. 메시지가 표시되면 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["콘솔 에이전트 생성"^].
...
| 자격 증명 편집 | 이 Cloud Volumes ONTAP 시스템에서 사용할 다른 Azure 자격 증명과 다른 Azure 구독을 선택할 수 있습니다. 종량제 Cloud Volumes ONTAP 시스템을 배포하려면 선택한 Azure 구독에 Azure Marketplace 구독을 연결해야 합니다. https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html["자격 증명을 추가하는 방법을 알아보세요"^]. 
|===
. *서비스*: Cloud Volumes ONTAP 과 함께 사용하거나 사용하지 않을 개별 서비스를 활성화하거나 비활성화합니다.
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["NetApp Data Classification 에 대해 자세히 알아보세요"^]
...
TIP: 사용 중인 Azure 계정에  https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["필요한 권한"^]이(가) 있는 경우, 배포 실패 또는 삭제 시 Console에서 리소스 그룹의 Cloud Volumes ONTAP 리소스를 제거합니다.
...
| 기존 사용 | 기존 보안 그룹을 선택하는 경우 해당 보안 그룹은 Cloud Volumes ONTAP 요구 사항을 충족해야 합니다. link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["기본 보안 그룹 보기"^]. 
|===
...
NOTE: 선택한 버전에 대해 최신 릴리스 후보, 일반 출시 또는 패치 릴리스가 있는 경우 콘솔은 작업 환경을 생성할 때 시스템을 해당 버전으로 업데이트합니다. 예를 들어 Cloud Volumes ONTAP 9.16.1 P3을 선택했는데 9.16.1 P4를 사용할 수 있는 경우 업데이트가 발생합니다. 단, 9.15에서 9.16으로의 업데이트와 같이 릴리스 간에는 업데이트가 발생하지 않습니다.
...
+
디스크 유형 및 크기 선택에 대한 도움말은 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Azure에서 시스템 크기 조정"^]을 참조하십시오.
...
+
이 옵션은 특정 VM 유형에서만 사용할 수 있습니다. 지원되는 VM 유형을 확인하려면 link:reference-configs-azure.html#ha-pairs["HA 쌍에 대한 라이선스별 지원 구성"]을 참조하십시오.
...
+
자세한 내용은 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["볼륨 사용 프로필 이해"^] 및 link:concept-data-tiering.html["데이터 계층화 개요"^]을 참조하십시오.
...
** 디스크 크기는 초기 집계의 모든 디스크와 간단한 프로비저닝 옵션을 사용할 때 콘솔이 생성하는 모든 추가 집계에 적용됩니다.  고급 할당 옵션을 사용하면 다른 디스크 크기를 사용하는 집계를 만들 수 있습니다.
+
디스크 크기 선택에 대한 도움말은 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Azure에서 시스템 크기 조정"^]을 참조하십시오.
...
+
link:reference-configs-azure.html#ha-pairs["Azure에서 지원되는 구성에 대해 자세히 알아보세요."^].
...
. *사용 프로필, 디스크 유형 및 계층화 정책*: 필요한 경우 스토리지 효율성 기능을 활성화할지 여부를 선택하고 볼륨 계층화 정책을 변경합니다.
+
자세한 내용은 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["볼륨 사용 프로필을 선택하세요"^], link:concept-data-tiering.html["데이터 계층화 개요"^], 및  https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB: CVO에서는 어떤 인라인 스토리지 효율성 기능이 지원되나요?"^]을 참조하십시오.
...
* link:task-deploy-cvo-azure-mktplc.html["Azure Marketplace에서 Azure에 Cloud Volumes ONTAP 배포"^]
