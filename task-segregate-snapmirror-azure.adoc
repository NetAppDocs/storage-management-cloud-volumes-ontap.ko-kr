...
---
= Azure의 Cloud Volumes ONTAP에서 SnapMirror 트래픽 분리
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
----
.. privateIPAddress를 저장하세요. 이 IP 주소<new_added_nic_primary_addr>는 <<step-second,새로운 NIC용 브로드캐스트 도메인 및 인터클러스터 LIF>>에서 클러스터 간 LIF를 생성하는 데 사용됩니다.
...
== [[step-second]]2단계: 새 NIC에 대한 새 IPspace, 브로드캐스트 도메인 및 인터클러스터 LIF 생성
