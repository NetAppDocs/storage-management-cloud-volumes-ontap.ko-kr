...
---
= Cloud Volumes ONTAP의 쓰기 속도에 대해 알아보기
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
link:reference-configs-aws.html["Cloud Volumes ONTAP 지원하는 Amazon EC2 인스턴스에 대해 자세히 알아보세요."].
...
HA 페어를 사용하는 경우, Cloud Volumes ONTAP 9.8 릴리스부터 여러 VM 유형에서 고속 쓰기를 지원합니다. 고속 쓰기를 지원하는 VM 유형을 보려면 link:reference-configs-azure.html["Cloud Volumes ONTAP 릴리스 노트"]을 참조하십시오.
...
HA 페어를 사용하는 경우, Cloud Volumes ONTAP 9.13.0 릴리스부터 여러 VM 유형에서 고속 쓰기를 지원합니다. 고속 쓰기를 지원하는 VM 유형을 보려면 link:reference-configs-gcp.html#supported-configurations-by-license["Cloud Volumes ONTAP 릴리스 노트"^]을 참조하십시오.

link:reference-configs-gcp.html["Cloud Volumes ONTAP 지원하는 Google Cloud 머신 유형에 대해 자세히 알아보세요."^].
